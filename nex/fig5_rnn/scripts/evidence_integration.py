...
from nex.fig5_runn.scripts.EI_train_utils import update_nested_dict
...
def train(config, save_dir, time_stamp, sync_wandb=False):
    ...
    lowers = {"input_weights": -0.2, "IonotropicSynapse_gS": 1e-10}
    uppers = {"input_weights": 0.2, "IonotropicSynapse_gS": np.max(abs(conn_matrix))*3}
    ...
    for i in tqdm(range(train_params["n_epochs"])):
        ...
        if i==0:
            ...
            print(f"grad-step time: {time.time() - start_time}")
    ...
    params = tf.forward(opt_params)
    final_gS = params[1]["IonotropicSynapse_gS"]
